...
def create_svg_from_spreadsheet(svg_file_path, file_path, column_size=800):
    ...
    begin_svg = f'<svg width="{column_size}" height="800" style="transform: rotate(-270deg);" xmlns="http://www.w3.org/2000/svg"> <defs> <linearGradient id="grad1" x1="0%" y1="0%" x2="100%" y2="0%"> <stop offset="1%" style="top-color:rgb(205,255,225);stop-opacity:.15" /> <stop offset="9%" style="top-color:rgba(5,5,225,.6);stop-opacity:.25" /> <stop offset="10%" style="stop-color:rgb(255,5,5);stop-opacity:.35" /> <stop offset="100%" style="stop-color:rgb(0,5,0);stop-opacity:.7" /> </linearGradient> </defs> '
    end_svg = '</svg>'
    ...
    for name in column_names:
        ...
        polyline_list = make_polyline(polyline_coordinates, multiplier=1.25, is_animation=False, peak_multiplier=-15)
        ...
        animate_element = (
            f'<animate attributeName="d" dur="5s" values="M{polyline_list};M{to_coords};M{polyline_list}" '
