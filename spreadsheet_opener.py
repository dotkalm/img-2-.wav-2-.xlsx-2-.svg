...
def create_svg_from_spreadsheet(svg_file_path, file_path, column_size=800):
    ...
    begin_svg = f'<svg width="{column_size}" height="800" xmlns="http://www.w3.org/2000/svg"> <defs> <linearGradient id="grad1" x1="0%" y1="0%" x2="100%" y2="0%"> <stop offset="1%" style="top-color:rgb(205,255,225);stop-opacity:.15" /> <stop offset="9%" style="top-color:rgba(5,5,225,.6);stop-opacity:.25" /> <stop offset="10%" style="stop-color:rgb(255,5,5);stop-opacity:.35" /> <stop offset="100%" style="stop-color:rgb(0,5,255);stop-opacity:.7" /> </linearGradient> </defs> ' 
    end_svg = '</svg>'
    ...
    for name in column_names:
        ...
        animate_element = (
            f'<animate attributeName="d" dur="12s" values="M{polyline_list};M{to_coords};M{polyline_list}" '
